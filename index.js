	...
	async getDeviceInfo() {
		try {
			...
			if (status) {
				...
				this.log(
					'info',
					`Connected to ${status.cardType || 'Makito X4 Encoder'} (S/N: ${status.serialNumber || 'Unknown'})`,
				)
			}
		} catch (error) {
				...
				for (let i = 0; i < 4; i++) {
					try {
						...
						if (this.presetListCounter && this.presetListCounter % 10 === 0) {
							...
							this.log(
								'debug',
								`Encoder ${i} thumbnail check - Counter: ${this.presetListCounter}, State: ${stats?.state}`,
							)
			...
			if (typeof stats.state !== 'undefined') {
				switch (stats.state) {
					case 0:
						...
						break
					case 3:
						stateStr = 'Awaiting Frame'
						break
					case 5:
						stateStr = 'Not Encoding'
						break
					case 7:
						stateStr = 'Working'
						break
